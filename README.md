...
As in the APD4F, the MLP-APD training data was standardized using the scikit-learn preprocessing module called StandardScaler. Therefore, it is also necessary to standardize the real-time data that is delivered to the imported model by applying the standardization formula detailed in the previous subsection.
...
```
On lines 993 (ADP4F with 2 APDs) and 778 (ADP4F with 3 APDs):
    #cap = cv2.VideoCapture(0)

On lines 994 (ADP4F with 2 APDs) and 779 (ADP4F with 3 APDs):
...
```
On lines 566 (ADP4F with 2 APDs) and 227 (ADP4F with 3 APDs):
    archivo = open("./label/results" + "_video_test" + ".txt", "a")
```
```
On lines 924 (ADP4F with 2 APDs) and 709 (ADP4F with 3 APDs):
